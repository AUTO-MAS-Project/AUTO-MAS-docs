---
title: 常见问题
description: AUTO_MAS 的常见问题
date: 2025-04-27
---

# 常见问题

更多问题请参考 <Pill name="AUTO_MAS GitHub Issues" :image="{ light: '/resource/github.svg', dark: '/resource/github-dark.svg', }" link="https://github.com/DLmaster361/AUTO_MAS/issues"/> 。
...
- AUTO_MAS初衷是MAA的调度工具，现在的目标是所有脚本软件的调度工具。
- AUTO_MAS项目组与MAA项目组是完全独立的两班人马。
- AUTO_MAS项目组与MAA项目组互相遵守开源协议和社区规范。

### **AUTO_MAS利好代肝吗？**

- 代肝用 AUTO_MAS 就是利好代肝，用户用 AUTO_MAS 就是利好用户。
- 而且 AUTO_MAS 的传播范围越广，就越利好用户，所以还不快帮 AUTO_MAS 宣传一波！

### 我的数据（账号密码）安全吗？

- AUTO_MAS完全在本地处理账号密码数据。
...
- 请在 AUTO_MAS 中进行 MAA 配置，并避免在配置期间重启MAA，以确保所有更改都被程序正常记录。

### **为什么 AUTO_MAS 固定到任务栏后无法正常运行？**

- AUTO_MAS 具有特殊的运行机制，导致了 Windows 系统无法正确定位到 AUTO_MAS 的实际路径。
- 为正确将软件固定到任务栏，您需要在创建任务栏快捷方式后，在 AUTO_MAS 图标 **右键** 然后在 **AUTO_MAS Component** 上 **右键 > 属性**，在 **目标** 一栏填入正确的 `AUTO_MAS.exe` 所在路径并 **确认**。
...
- 请确认调度队列的 **定时运行** 已 **开启**，并且 **软件未被意外关闭**，任何睡眠休眠等情况均无法运行AUTO_MAS（见下）。
...
以下是 AUTO_MAS 的 **自动代理** 相关策略，了解这些信息有助于您理解 AUTO_MAS 的行为，以便您定制自己的调度方案

#### 用户配置说明

以下为 **自动代理** 模式下，AUTO_MAS 的配置策略。

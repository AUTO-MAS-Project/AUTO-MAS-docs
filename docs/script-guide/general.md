...
三月七、SRC、zzzOD、M9A 等常见脚本都有成熟模板，先去看看有没有你要的。若遇到问题，欢迎加用户交流群，与开发者、模板贡献者交流。
:::
...
这些设置决定 AUTO-MAS 如何调度你的脚本，填得是否准确直接影响代理的稳定性。
...
- **有这个文件夹**：进去挑一个 `.log` 或 `.txt` 文件。
  - 文件名里**不带日期**（如 `log.txt`、`gui.log`）→ 直接选它即可。
...
告诉 AUTO-MAS 每行日志中时间戳的起止位置——AUTO-MAS 依据时间戳判断脚本是否卡住。
...
把上面那段时间戳的写法转换成符号，AUTO-MAS 才能解析出时间。
